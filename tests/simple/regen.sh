...
for db in db1 db2 db3 db4 db5; do
    psql -q -d $db -c "update pgq.queue set queue_ticker_idle_period='2 seconds'"
done
...
for db in db2 db3 db4 db5; do
  ...
  run londiste $v conf/londiste_$db.ini add-table mytable --create-full
done

msg "Register another table to test unregistration"
run psql -d db1 -c "create table mytable2 (id int4 primary key, data text)"
run londiste $v conf/londiste_db1.ini add-table mytable2
for db in db2 db3 db4 db5; do
  run londiste $v conf/londiste_$db.ini add-table mytable2 --create
done

msg "Wait until tables are in sync on db5"

run londiste conf/londiste_db5.ini wait-sync

msg "Unregister table2 from root"
run londiste $v conf/londiste_db1.ini remove-table mytable2

msg "Wait until unregister reaches db5"
run londiste conf/londiste_db5.ini wait-root



run londiste conf/londiste_db5.ini status

msg "Test skipped copy"
run londiste $v conf/londiste_db1.ini add-table mytable2
run londiste $v conf/londiste_db5.ini wait-root
run londiste $v conf/londiste_db5.ini add-table mytable2 --find-copy-node
run londiste $v conf/londiste_db5.ini wait-sync
...
run londiste $v conf/londiste_db2.ini worker -s
run sleep 20
...
run londiste $v conf/londiste_db2.ini wait-provider

msg "Promoting db2 to root"
run londiste $v conf/londiste_db2.ini takeover node1 --dead-root
run londiste $v conf/londiste_db2.ini tag-dead node1
...
run londiste $v conf/londiste_db3.ini repair --apply mytable
msg "Test compare"
run londiste conf/londiste_db3.ini compare mytable

#msg "Test branch compare"
#run londiste conf/londiste_db5.ini compare
#msg "Test branch repair"
#run londiste conf/londiste_db5.ini repair
